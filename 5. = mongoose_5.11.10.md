...
## Какие модели в mongoose задекларированы
mongoose.connection.models        //{ Kitt: Model { Kitt }, Dogg: Model { Dogg } }
...
Имя коллекции - имя документа, ПЕРЕДЕЛЫВАЯ имя модели через приведение его к нижнему регистру и множественному числу(!).



## a) Создание документа, где модель- как конструктор.
Для добавления в коллекцию НОВЫХ ЧЛЕНОВ используем ЭТОТ ЖЕ silenceInstance, меняя только в нем значения полей.  ??
...
const silenceInstance = new KittenModel({     //silenceinstances- это будет КОЛЛЕКЦИЯ в базе KittenModel.
  name: 'Noise',
  biography: 'Oblom',
  twitter: 'sw.ru',
});
...
=>> mongoose при создании КОЛЛЕКЦИИ в mongoDb отталкивается от имени ДОКУМЕНТА, ПЕРЕДЕЛЫВАЯ имя через приведение его к нижнему регистру и множественному числу(!).
Если теперь мы выполним в консоли mongodb (войти в нее - командой mongo) команду «show collections», то увидим, что была создана коллекция users.
